...
export const getStats = async (req, res) => {
    try {
        const [
            ...
            // ✅ FIXED: Fetch the 'products' relation to get the price
            supabase.from('purchases').select('products(price)')
        ]);
        ...
        const totalRevenueCalc = allSales?.reduce((sum, item) => {
            // The data comes back as: { products: { price: 100 } }
            // We use optional chaining (?.) just in case a product was deleted
        ...
        return res.json({
            stats: {
                ...
                totalRevenue: totalRevenueCalc, // Use the calculated value
            },

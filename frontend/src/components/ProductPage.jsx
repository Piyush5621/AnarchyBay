...
export default function ProductPage() {
  ...
  const handleMockPurchase = async () => {
    ...
    setBuying(true); // Reusing your existing loading state

    try {
      const token = getAccessToken();
      const res = await fetch(`${API_URL}/api/purchases/dev/mock-buy`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) throw new Error(data.error || "Mock buy failed");

      toast.success("🦄 Mock Purchase Successful!");

      // Navigate to library to verify it appears
      navigate("/library");
                ...
                <div className="space-y-3">
                  ...
                  {import.meta.env.MODE === 'development' && (
                    <button
                      ...
                      className="w-full mt-4 py-3 bg-gray-800 text-white font-mono text-xs uppercase border-2 border-black border-dashed hover:bg-black transition-all"
                    >
                      🛠️ Dev Mode: Instant Add to Library
                    </button>
                  )}
